...
func (c *LocalTestingClient) GetInstance(ctx context.Context, localInstanceId string, auth_token string) (*GetLocalBinaryInstanceResponse, error) {
	...
	if err := c.http.Get(ctx, "/local/v1/" + url.PathEscape(localInstanceId), map[string]string{"auth_token": auth_token}, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
...
func (c *LocalTestingClient) DisconnectInstance(ctx context.Context, localInstanceId string, auth_token string) (*DisconnectLocalBinaryInstanceResponse, error) {
	...
	if err := c.http.Delete(ctx, "/local/v1/" + url.PathEscape(localInstanceId), map[string]string{"auth_token": auth_token}, &out); err != nil {
		return nil, err
	}
	return &out, nil
}
